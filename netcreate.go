...
func netCreateMain(ctx context.Context, args []string) error {
	...
	if nameFlag == "" {
		fmt.Fprintf(os.Stderr, "npte net create: --name is required\n")
		fmt.Fprintf(os.Stderr, "npte net create: try `npte net create --help' for more help.\n")
		os.Exit(2)
	}
	...
	if err := validateEndpointName(state.Prefix, nameFlag); err != nil {
		fmt.Fprintf(os.Stderr, "npte net create: %s\n", err)
		os.Exit(2)
	}

	if _, exists := state.Hosts[nameFlag]; exists {
		fmt.Fprintf(os.Stderr, "npte net create: host '%s' already exists\n", nameFlag)
		os.Exit(1)
	}

	// Auto-allocate subnet: 10.0.<index>.0/24
	subnet := fmt.Sprintf("10.0.%d.0/24", state.NextSubnetIndex)
	_, ipNet := runtimex.LogFatalOnError2(net.ParseCIDR(subnet))
	...
	fmt.Fprintf(os.Stderr, "npte: allocate subnet %s for endpoint '%s'\n", subnet, nameFlag)

	// Save state early so that `npte net destroy` can clean up partial initialization
	fmt.Fprintf(os.Stderr, "npte: save updated state to %s\n", netStatePath)
	state.Hosts[nameFlag] = &hostState{
		Name:   nameFlag,
		Subnet: subnet,
	}
	state.NextSubnetIndex++
	runtimex.LogFatalOnError0(saveNetState(state))

	// Create the endpoint namespace
	fmt.Fprintf(os.Stderr, "npte: create network namespace '%s'\n", ns)
	...
	fmt.Fprintf(os.Stderr, "npte: configure endpoint side (%s/%s on %s)\n", endpointAddr, cidr, endpointVeth)
	...
	fmt.Fprintf(os.Stderr, "npte: tune TCP buffer sizes inside '%s'\n", ns)
	...
	fmt.Fprintf(os.Stderr, "npte: created '%s' with address %s\n", nameFlag, endpointAddr)
	return nil
}
