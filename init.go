...
func initMain(ctx context.Context, args []string) error {
	...
	fmt.Fprintf(os.Stderr, "npte: initialize or reinitialize the local '.npte' dir\n")
	fmt.Fprintf(os.Stderr, "npte: ensure the required directories exist\n")
	dirs := []string{
	...
	if _, err := os.Stat(namePath); os.IsNotExist(err) {
		fmt.Fprintf(os.Stderr, "npte: creating default config file: %s\n", namePath)
		runtimex.LogFatalOnError0(os.WriteFile(namePath, []byte("npte\n"), 0644))
	}

	fmt.Fprintf(os.Stderr, "npte: done\n")
	return nil
}
