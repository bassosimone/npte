...
func initMain(ctx context.Context, args []string) error {
	...
	logDetails("npte: initialize or reinitialize the local '.npte' dir\n")
	logDetails("npte: ensure the required directories exist\n")
	dirs := []string{
	...
	if _, err := env.Stat(namePath); os.IsNotExist(err) {
		logDetails("npte: creating default config file: %s\n", namePath)
		env.LogFatalOnError0(env.WriteFile(namePath, []byte("npte\n"), 0644))
	}

	logDetails("npte: done\n")
	return nil
}
